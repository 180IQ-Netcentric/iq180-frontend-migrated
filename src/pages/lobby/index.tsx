...
const Lobby = () => {
  ...
  const [showMaxPlayerError, setShowMaxPlayerError] = useState(false)
  ...
  useEffect(() => {
    ...
    if (playerCount === undefined) {
      newSocket.emit('showAllPlayers')
      newSocket.on('onShowAllPlayers', (playerInfos: PlayerInfos) => {
        const playerIndex = playerInfos.findIndex(
          (player) => player.id === newSocket.id
        )
        setPlayerCount(playerInfos.length)
        if (playerInfos.length >= 2 && playerIndex < 0) {
          socket?.disconnect()
          newSocket.disconnect()
          setShowMaxPlayerError(true)
        }
      })
    }
  ...
  return (
    <>
      <ErrorAlert
        open={showMaxPlayerError}
        setOpen={setShowMaxPlayerError}
        title={t('109')}
        description={t('110')}
        primaryAction={() => history.push('/')}
        customOnClose={() => history.push('/')}
      />
      <GameContainer>
